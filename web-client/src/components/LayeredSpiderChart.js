...
// spider chart with multiple solid layers, assumes (best >= average >= worst) for all keys
const LayeredSpiderChart = ({ width, height, style, data, axisMaximums }) => {
    ...
    const averageIndicatorPoints = data.map((point, index) => polarToCartesian(2 * Math.PI * (index / data.length), 9 * (Math.min(point.average / axisMaximums[point.key], 1) || 0.001)))
    ...
    const worstIndicatorPoints = data.map((point, index) => polarToCartesian(2 * Math.PI * (index / data.length), 9 * (Math.min(point.worst / axisMaximums[point.key], 1) || 0.001)))
...
export default LayeredSpiderChart